...
# SELECTORS
# attrition = ["0", "1"]
# attrition_selection = st.sidebar.selectbox('Attrition', attrition)
...
with container1:
    with col1:
        ...
        st.write("#### Employee age distribution")
       
               
        # 1_HISTOGRAM
        ax = figs[0].add_subplot(1,1,1) 
        sns.histplot(data=ibm, x='Age', multiple="dodge", shrink=0.5, ax = ax)
        ax.set(xlabel="Age", ylabel = "Employee count", xlim= (15,60), ylim= (0, 175))
        st.write(figs[0])
